...
void glu_dev_fill_full_screen( const gluColor_t color){
#if   ( RH_CFG_GRAPHIC_COLOR_TYPE == RH_CFG_GRAPHIC_COLOR_BIN    )

    GLU_TYPE(Pixel) pixel = COLOR_1BIT(color);
    if( pixel == 0x00 ){
    ...
    GLU_TYPE(Pixel) pixel = COLOR_16BIT(color);
    if( sizeof(uint16_t)==sizeof(wchar_t) ){
        wmemset( (wchar_t*)canvas.ptr, pixel, RH_CFG_SCREEN_HEIGHT*RH_CFG_SCREEN_WIDTH );
    }else{
    ...
    GLU_TYPE(Pixel) pixel = COLOR_24BIT(color);
